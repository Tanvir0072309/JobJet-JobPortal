...
// visually with Small (both are on the smaller end), and an unknown size
// simply shows no tag rather than a misleading guess.
const SIZE_TAG_STYLES: Record<string, { bg: string; text: string; label: string }> = {
  startup: { bg: "#FFE8D6", text: "#C2540A", label: "Small" },
  small: { bg: "#FFE8D6", text: "#C2540A", label: "Small" },
  mid: { bg: "#FFF3C4", text: "#8A6D00", label: "Mid" },
  large: { bg: "#D8F5DE", text: "#137333", label: "Large" },
};

function SizeTag({ size }: { size?: string | null }) {
  const style = size ? SIZE_TAG_STYLES[size] : null;
  if (!style) return null;
  return (
    <View style={[sizeTagStyles.pill, { backgroundColor: style.bg }]}>
      <Text style={[sizeTagStyles.text, { color: style.text }]}>{style.label}</Text>
    </View>
  );
}

const sizeTagStyles = StyleSheet.create({
...
export default function FindCompaniesScreen() {
  ...
  const [interestedPosts, setInterestedPosts] = useState<string[]>([]);
  ...
  useEffect(() => {
    getProfile()
      .then(({ profile }) => {
        ...
        setInterestedPosts(Array.isArray(profile?.interested_posts) ? profile.interested_posts : []);
      })
      .catch(() => {
  ...
  // A company only ever gets a "Matches your interest" callout when one of
  // its open-role titles actually overlaps with the candidate's own
  // Interested Posts (see Profile) - never a generic guess, and never shown
  // at all when there's no real match.
  const matchedPost = useCallback(
    (company: Company): string | null => {
      if (!interestedPosts.length || !company.jobs?.length) return null;
      for (const job of company.jobs) {
        const title = String(job?.title || "").toLowerCase();
        if (!title) continue;
        const hit = interestedPosts.find(
          (post) => title.includes(post.toLowerCase()) || post.toLowerCase().includes(title)
        );
        if (hit) return job.title;
      }
      return null;
    },
    [interestedPosts]
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // pile of every company ever searched for.
      setCompanies(res.companies || []);
      setCached(CACHE_KEY, res.companies || []);
    } catch (err) {
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {companies.length === 0 ? (
        ...
        companies.map((company) => {
          const match = matchedPost(company);
          return (
            <Pressable
              key={company.id}
            ...
            >
              <View style={styles.companyHeaderRow}>
                ...
                <View style={{ flex: 1 }} />
                <SizeTag size={company.company_size} />
              </View>
              ...
              {/* Only ever shown when a real overlap with the candidate's own
                  Interested Posts (Profile) exists - never a generic "no match" line. */}
              {match ? (
                <View style={styles.matchBadge}>
                  <Text style={styles.matchBadgeText}>Matches your interest: {match}</Text>
                </View>
              ) : null}
            </Pressable>
          );
...
const styles = StyleSheet.create({
  ...
  matchBadgeText: { ...typography.tiny, color: colors.primary, fontWeight: "700" },
});
