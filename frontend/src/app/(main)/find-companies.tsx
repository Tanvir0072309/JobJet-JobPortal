...
export default function FindCompaniesScreen() {
  ...
  const handleSearch = async () => {
    ...
    setNotice(null);
    try {
      // Groq does the actual discovery server-side (name, website, career
      // page, example open roles) - no client-side geocoding needed anymore.
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      {companies.length === 0 ? (
        <EmptyState
          ...
          description="Search a location above to discover companies. Names, career pages, and example roles are found for you by Groq AI — add your Groq API key in Settings if you haven't yet."
        />
      ) : (
...
const styles = StyleSheet.create({
  ...
  companyCardSelected: { borderColor: colors.primary, backgroundColor: colors.primaryMuted },
