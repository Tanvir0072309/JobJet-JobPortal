...
const listDocuments = asyncHandler(async (req, res) => {
  const result = await db.query(
    "SELECT id, name, document_type, file_type, file_size_bytes, is_default, created_at, updated_at FROM documents WHERE user_id = $1 ORDER BY created_at DESC",
    [req.user.id]
  );
...
const uploadDocument = asyncHandler(async (req, res) => {
  ...
  const result = await db.query(
    `INSERT INTO documents (user_id, name, document_type, file_type, file_path, file_size_bytes)
     VALUES ($1, $2, $3, $4, $5, $6)
     RETURNING id, name, document_type, file_type, file_size_bytes, is_default, created_at`,
    [
      req.user.id,
      displayName,
      documentType,
